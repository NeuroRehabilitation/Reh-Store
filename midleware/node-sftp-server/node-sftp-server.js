      ...
      return function (symbol) {
        return (_this[methodname] = function () {
          ...
          return _this.sftpStream.status(
            _this.req,
            ssh2.utils.sftp.STATUS_CODE[symbol]
          );
        });
      };
    })(this);
...
var SFTPServer = (function (superClass) {
  ...
  function SFTPServer(options) {
    ...
    switch (
      options.privateKeyFile.split(".")[
        options.privateKeyFile.split(".").length - 1
      ]
    ) {
      case "pem":
        const privateKeyPEM = fs.readFileSync(options.privateKeyFile);
        options.privateKeyFile = crypto
          .createPrivateKey({
            key: privateKeyPEM,
            format: "pem",
          })
        ...
        break;
      case "key":
        options.privateKeyFile = fs.readFileSync(options.privateKeyFile);
        break;
      default:
        break;
    }
    ...
    this.server = new ssh2.Server(
      {
        hostKeys: [options.privateKeyFile],
      },
      (function (_this) {
...
var Statter = (function () {
  ...
  Statter.prototype.nofile = function () {
    return this.sftpStream.status(
      this.reqid,
      ssh2.utils.sftp.STATUS_CODE.NO_SUCH_FILE
    );
  };
...
var SFTPSession = (function (superClass) {
  ...
  // TODO: Traduzido!
  SFTPSession.prototype.closeSession = function () {
    //Editado manualmente
    this.sftpStream.end();
  };
  // TODO: Traduzido!
      ...
      return fs.stat(
        ...
        function (err, stats) {
          ...
          if (offset >= stats.size) {
            return this.sftpStream.status(
              reqid,
              ssh2.utils.sftp.STATUS_CODE.EOF
            );
          } else {
    ...
    if (this.handles[handle]) {
      switch (this.handles[handle].mode) {
        ...
        default:
          return this.sftpStream.status(
            reqid,
            ssh2.utils.sftp.STATUS_CODE.FAILURE
          );
      }
    }
  };
  ...
  SFTPSession.prototype.RENAME = function (reqid, oldPath, newPath) {
    return this.emit(
      ...
      oldPath,
      newPath,
      new Responder(this.sftpStream, reqid)
    );
  };
  // TODO: Traduzido!
